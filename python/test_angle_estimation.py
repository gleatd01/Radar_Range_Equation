...
def test_angle_estimation():
    ...
    try:
        ...
        print(f"✓ solve.calculate_Theta({theta_B}) = {Theta}")

        # Test v_phi_full
        result = RRE.solve.v_phi_full(5.0, 5.0, 10.0)
        assert isclose(result, 1.0, rel_tol=1e-9), f"v_phi_full at center should be ~1.0, got {result}"
        print(f"✓ solve.v_phi_full(5.0, 5.0, 10.0) = {result}")

        # Test estimate_phi_hat
        phi_hat = RRE.solve.estimate_phi_hat(1.0, 1.0, 10.0, 5.0)
        ...
        print(f"✓ solve.estimate_phi_hat(1.0, 1.0, 10.0, 5.0) = {phi_hat}")
        ...
        print(f"✓ solve.db_to_linear({S_N_dB}) = {S_N_linear}")

        # Test sigma_phi_amplitude (from problem statement page 2)
        theta_B_1 = 10.0
        phi_s_1 = 5.0
        S_N_1 = 10.0
        sigma_phi_amp = RRE.solve.sigma_phi_amplitude(theta_B_1, S_N_1, phi_s_1)
        ...
        print(f"✓ solve.sigma_phi_amplitude({theta_B_1}, {S_N_1}, {phi_s_1}) = {sigma_phi_amp:.4f} degrees")
        ...
        S_N_dB_2 = 8
        S_N_linear_2 = RRE.solve.db_to_linear(S_N_dB_2)  # approx 6.3
        d_1 = 2.0  # 2 meters
        sigma_phi_phase_val = RRE.solve.sigma_phi_phase(lambda_, d_1, S_N_linear_2)
        ...
        print(f"✓ solve.sigma_phi_phase({lambda_}, {d_1}, {S_N_linear_2:.2f}) = {sigma_phi_phase_val:.6f} radians")
        ...
        B_1 = 200e6  # 200 MHz
        sigma_phi_time_val = RRE.solve.sigma_phi_time(c, d_1, B_1)
        ...
        print(f"✓ solve.sigma_phi_time({c:.2e}, {d_1}, {B_1:.2e}) = {sigma_phi_time_val:.4f} radians")
